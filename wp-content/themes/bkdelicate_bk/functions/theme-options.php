<?php 
require ($include_path . 'settings-theme.php');
...
<br/>
<p>Find more information about this <a href="<?php echo $manualurl; ?>">theme configuration</a> in the docs. </p>
</div>
		...
		for ($j = 0; $j < count($tcontrols); $j++) {
            ...
            if(isset($tcontrols[$j]['mode']) && $tcontrols[$j]['mode'] == 'dimensions') {
              ...
              nat_th($tcontrols[$j]['title']);	
					...
					case 'upload':
						nat_th($tcontrols[$j]['title']);
						nat_upload($tcontrols[$j]['name'], stripslashes(get_option($tcontrols[$j]['name'])));
						nat_hit($tcontrols[$j]['desc']);	
						nat_endth();
						break;			
					case 'sort-item':
