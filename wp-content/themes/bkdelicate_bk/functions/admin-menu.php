...
function nattywp_add_admin() {
    ...
    global $themename, $options, $query_string;     
	$page_color = false;    
   
// Check all the Options, then if the no options are created for a ralitive sub-page... it's not created.    

	/*add_menu_page('Page Title', $themename, 'manage_options','nattywp_home', 'nattywp_page_gen', get_bloginfo('template_url').'/functions/js/favicon.png', 61);
    add_submenu_page('nattywp_home', $themename, 'Theme Options', 'manage_options', 'nattywp_home','nattywp_page_gen'); // Default
    add_submenu_page('nattywp_home', $themename, 'Color', 'manage_options', 'nattywp_color', 'nattywp_color');      	
    add_submenu_page('nattywp_home', $themename, 'More themes', 'manage_options', 'more-nattywp', 'nattywp_more_themes_page'); 
  */
...
global $options, $themename, $manualurl, $include_path, $current;  
    ...
    <div class="header">
      <div class="main-top">
        <div class="general-pad">
         <div class="left">
            <h1><?php echo $themename; ?> &rarr; <span class="blu">Theme Options</span></h1>
            <small>Theme Version: <?php echo $current; ?></small>
         </div>
         <div class="right">
      ...
      <div class="supportline">
       <div class="general-pad-two">
        <div class="left">
          <a class="ico-docs" href="<?php echo $manualurl; ?>">Theme Documentation</a>
          <a class="ico-support" href="http://support.nattywp.com">Submit a Ticket <small>(register first)</small></a>
        </div>
        <div class="right">
             	<img style="display:none" src="<?php echo get_template_directory_uri(); ?>/functions/images/loading.gif" class="ajax-loading-img ajax-loading-img-top" alt="Working..." /><input type="submit" value="<?php _e('Update Options') ?> &raquo;" class="button button-primary submit-button natty-options-submit" />
        </div>
        <div class="clear"></div>
        </div>
      </div>
...
<div id="main"<?php if($page == 'color') {echo ' class="color-page"';} else { echo ' class="settings-page"';} ?>> 
  ...
  global $functions_path;
	  if($page == 'color') {
	  	include ($functions_path . 'theme-color.php');
	  } else {			
      include ($functions_path . 'theme-options.php');  			
	  }
	?>
	<div class="clear"></div>
</div> <!-- end main -->
...
<div class="footer">
  <div class="general-pad-two">
    <div class="right">
      <input type="submit" class="natty-options-submit button-primary" name="Submit" value="<?php _e('Update Options') ?> &raquo;" />
    </div>
	<div class="clear"></div>
	</div>
</div>
    	
   <p class="submit"><input type="submit" class="natty-backup-submit" name="backup-submit" value="<?php _e('Backup Options') ?> &raquo;" /></p>	
   
   <?php 
	  if($page == 'color') { ?>
	  	<p class="submit"><input type="submit" class="natty-restore-submit<?php if(get_option($themename.'_color_settings_back') == '') {echo ' no-but';} ?>" name="restore-color" value="<?php _e('Restore Color Options') ?> &raquo;" /></p>
	 <?php } else {	 ?>		
      <p class="submit"><input type="submit" class="natty-restore-submit<?php if(get_option($themename.'_settings_back') == '') {echo ' no-but';} ?>" name="restore-options" value="<?php _e('Restore Theme Options') ?> &raquo;" /></p>		
	 <?php }	?>   
</form>
...
function natty_ajax_callback() {
  global $controls, $tcontrols, $themename;
  ...
  if ($save_type == 'theme_options') {
      ...
      update_option($themename.'_settings', $output);      
   }  
  if ($save_type == 'color_options') {
      $data = $_POST['data'];
      parse_str($data,$output);  
      update_option($themename.'_color_settings', $output);
   }
  if ($save_type == 'theme_options-backup') {
      $data = $_POST['data'];     
      parse_str($data,$output);       
      update_option($themename.'_settings_back', $output);      
   }
  if ($save_type == 'theme_options-restore') {
      $tmp = get_option($themename.'_settings_back');   
      update_option($themename.'_settings', $tmp);      
   }  
  if ($save_type == 'color_options-backup') {
      $data = $_POST['data'];
      parse_str($data,$output);  
      update_option($themename.'_color_settings_back', $output);
   }
  if ($save_type == 'color_options-restore') {
      $tmp = get_option($themename.'_color_settings_back');   
      update_option($themename.'_color_settings', $tmp); 
   }
    die();
}
?>